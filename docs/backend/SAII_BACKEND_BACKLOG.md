...
- **Cambios principales:**
  ...
  * **Reordenamiento de Ciclo de Vida CSRF**: Se movió la inicialización del token CSRF en `public/index.php` para ejecutarse antes de la función de optimización de rendimiento `session_write_close()`. Esto garantiza que los tokens generados se guarden efectivamente en el disco y no causen rechazos de token inválido en peticiones POST subsiguientes.
  * **Robustez en Parsing de Reportes**: Se implementó una cláusula `try-catch` alrededor de la des-serialización JSON en `mappers.savedReport` (`public/js/data.js`), previniendo caídas catastróficas de la aplicación ante inconsistencias o datos corruptos en el campo `query_config` de la tabla `saved_reports`.
  * **Mapeo de Roles y Prevención de Escape HTML**: Se decodificó el campo `permissions` de roles en el servidor dentro de `public/index.php` antes de pasarlo al controlador de salida. Esto evita que `BaseController` escape las comillas dobles en formato `&quot;`. También se añadió un fallback con `.replace(/&quot;/g, '"')` en JavaScript y se mapeó `id` a `key_name` para que el frontend reconozca la sesión del usuario administrador de forma exitosa.
